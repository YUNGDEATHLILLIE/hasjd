...
exports.run = async(client, message, args) => {
    ...
    rolVer(victim.id, ayarlar.guildRoles.kizRoles);
    setTimeout(() => {
        rolAl(victim.id, ayarlar.guildRoles.autoRole)
    }, ayarlar.guildRoles.erkekRoles.length * 3001)

    if (!isim) isim = "Belirtilmemiş"
    message.channel.send(shxozpa.setColor(renk(0)).setDescription(`${victim} sunucumuza başarıyla <@&${ayarlar.guildRoles.kizRoles[0]}> rolüyle, \`${isim}\` ismiyle kaydedildi.`))
    message.react("✅")
}
...
exports.commandSettings = {
    ...
    description: "Sunucuya kız olarak kaydeder"
}
